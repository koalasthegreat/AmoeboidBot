...
async def on_message(message):
    ...
    if len(cards) == 0:
        ...
        img = image_manip.bytes_to_discfile(card.normal_image_bytes, "card.jpg")
        ...
        if len(images) <= 5:
            parsed_image = image_manip.stitch_images_horz(
                images, buf_horz=10, buf_vert=10
            )

        else:
            top = image_manip.stitch_images_horz(images[:5], buf_horz=10, buf_vert=5)
            bottom = image_manip.stitch_images_horz(images[5:], buf_horz=10, buf_vert=5)

            parsed_image = image_manip.stitch_images_vert([top, bottom])

        parsed_image_bytes = image_manip.img_to_bytearray(parsed_image)
        parsed_image_file = image_manip.bytes_to_discfile(
            parsed_image_bytes, "cards.jpg"
        )
